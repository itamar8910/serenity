...
#undef CPP_SEMANTIC_HIGHLIGHTER_DEBUG
#define CPP_SEMANTIC_HIGHLIGHTER_DEBUG 1
...
void SemanticSyntaxHighlighter::rehighlight(Palette const& palette)
{
    ...
    auto text = m_client->get_text();
    {
        ...
        // TODO: Maybe add a "*" marker to tokens in the edited range? to avoid the diff picking unchanged tokens as the ones that changed.
    ...
    m_last_edited_range.clear();

    update_spans(new_tokens_info);
}
...
void SemanticSyntaxHighlighter::update_spans(Vector<GUI::AutocompleteProvider::TokenInfo> const& tokens_info)
{
    VERIFY(m_current_palette.has_value());
    ...
    for (auto& token : tokens_info) {
        ...
        auto style = style_for_token_type(*m_current_palette, token.type);
...
void SemanticSyntaxHighlighter::set_last_edited_range(GUI::TextRange range, EditType edit_type)
{
    m_last_edit_type = edit_type;
    m_last_edited_range = range;
    if (range.end().column() != 0) {
        m_last_edited_range->set_end(GUI::TextPosition { range.end().line(), range.end().column() - 1 });
        return;
    }
    if (range.end().line() == 0)
        return;

    auto previous_line_length = m_client->get_document().line(range.end().line() - 1).length();

    m_last_edited_range->set_end(GUI::TextPosition { range.end().line() - 1, previous_line_length == 0 ? 0 : previous_line_length - 1 });
}
